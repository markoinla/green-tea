...
export function ChatHeader({
  ...
  return (
    <div className={cn('py-3 border-b border-border/40', documentId ? 'px-3' : 'px-6')}>
      {/* Centered to match the message list / input width in full-width mode. */}
      <div className={cn(!documentId && 'mx-auto w-full max-w-[820px]')}>
        {/* Tab bar */}
        <div className="flex items-center gap-1.5 mb-2 min-h-[28px] overflow-x-auto">
              ...
              <Tooltip key={conv.id}>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={cn(
                      'group relative flex items-center gap-1.5 h-7 px-2.5 rounded-md text-xs font-medium transition-all max-w-[140px] flex-shrink-0',
        ...
        <div className="flex items-center justify-between gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-1.5 text-xs font-medium text-foreground/80 hover:text-foreground transition-colors outline-none px-2 py-1.5 rounded-md hover:bg-muted/50 min-w-0">
