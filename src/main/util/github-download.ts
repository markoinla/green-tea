...
/**
 * Recursively download a GitHub directory (via the Contents API) into
 * `targetDir/<name>`, where `<name>` is the last path segment (or the repo name
 * for a repo-root URL). Returns the created subdir name. Generic and
 * skill-agnostic — used by the skills and plugins installers alike.
 */
export async function downloadFromGitHub(url: string, targetDir: string): Promise<string> {
  const { owner, repo, branch, path } = parseGitHubUrl(url)

  // For repo root URLs, the name is the repo name; for paths, it's the last segment
  ...
  await downloadDirectory(apiUrl, dir, defaultHeaders)
